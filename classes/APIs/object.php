...
class ezpObject
{
            ...
            default:
                if ( isset( $this->dataMap[$name] ) )
                {
                    ...
                    switch( $attribute->attribute( 'data_type_string' ) )
                    {
                        ...
                        case 'ezxmltext':
                            if( $this->validateXMLBlock( $value, $attribute, $this->object ) )
                                $attribute->store();
                            break;
    ...
    public function addTranslation( $newLanguageCode, $translationData )
    {
        ...
        foreach ( $translationData as $attr => $value )
        {
            ...
            {
                if( $this->validateXMLBlock( $value, $versionDataMap[$attr], $this->object ) )
    ...
    private function validateXMLBlock( $text, &$contentObjectAttribute, &$contentObject )
    {
        /*
            Taken from eZOEXMLInput::validateInput()
        */
        ...
        {
            \eZOEXMLInput::$showEmbedValidationErrors = true;
            $contentObjectAttribute->setValidationError( ezi18n( 'design/standard/ezoe/handler','Some objects used in embed(-inline) tags have been deleted and are no longer available.' ) );
        ...
        {
            \eZOEXMLInput::updateUrlObjectLinks( $contentObjectAttribute, $urlIDArray );
        }

        //$contentObject = $contentObjectAttribute->attribute( 'object' );
        // This ensures that we have a clean list if some relations were removed
        // However, it might have a negative effect if there are relations in other fields
        // We know that in this case, image slideshow elements don't have other relations
        $contentObject->resetInputRelationList();
